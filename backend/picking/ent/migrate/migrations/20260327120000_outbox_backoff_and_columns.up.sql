...
ALTER TABLE picking.event_outbox
...
DROP INDEX IF EXISTS picking.entityeventsoutbox_published_at_created_at;
...
CREATE INDEX IF NOT EXISTS idx_event_outbox_polling_v2
    ON picking.event_outbox (next_retry_at NULLS FIRST, correlation_id, created_at)
    WHERE published_at IS NULL AND dead_at IS NULL;

-- Index for tenant-based queries.
CREATE INDEX IF NOT EXISTS idx_event_outbox_tenant
    ON picking.event_outbox (tenant_id, created_at)
    WHERE published_at IS NULL AND dead_at IS NULL;
