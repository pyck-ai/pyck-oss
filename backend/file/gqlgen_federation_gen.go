...
func (ec *executionContext) resolveEntity(
	...
	case "Supplier":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Supplier": %w`, err)
		}
		switch resolverName {
...
func entityResolverNameForPickingOrder(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for PickingOrder", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
...
func entityResolverNameForSupplier(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Supplier", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
