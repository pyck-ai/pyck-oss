...
import (
	...
	entidempotencykey "github.com/pyck-ai/pyck/backend/inventory/ent/gen/idempotencykey"
)

// idempotencyStore is the inventory-service implementation of
// [idempotency.Store]. Lookup and Prune use the long-lived Ent client;
// the in-transaction writes (InsertInFlight, MarkCommitted) fetch the
// per-request *ent.Tx from ctx so they execute inside the same writer
// transaction the gqltx middleware opened for the mutation.
