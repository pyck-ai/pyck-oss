...
func main() {
	...
	// Set up Inventory Stock Service
	inventoryStockService, err := services.NewInventoryStockService()
	if err != nil {
		log.ForContext(ctx).Fatal().
			Err(err).
			Msg("failed setting up inventory stock service")
		return
	}

	defer inventoryStockService.Close()
	...
	resolver := resolvers.NewResolver(serviceName, dbClient, dataTypeValidator, inventoryStockService)
	...
	httpRouter.Handle("/health", handlers.NewHealthCheckHandler(db.NewDbHealthChecker(pgxDriver.DB(), entitem.Table)))
