...
func (ec *executionContext) resolveEntity(
	...
	case "InventoryItem":
		...
		case "findInventoryItemBySku":
			id0, err := ec.unmarshalNString2string(ctx, rep["sku"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findInventoryItemBySku(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindInventoryItemBySku(ctx, id0)
			if err != nil {
	...
	case "PickingOrderItem":
		resolverName, err := entityResolverNameForPickingOrderItem(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "PickingOrderItem": %w`, err)
		}
		switch resolverName {
	...
	case "ReplenishmentOrderItem":
		resolverName, err := entityResolverNameForReplenishmentOrderItem(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "ReplenishmentOrderItem": %w`, err)
		}
		switch resolverName {

		case "findReplenishmentOrderItemByID":
			id0, err := ec.unmarshalNID2githubᚗcomᚋgoogleᚋuuidᚐUUID(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findReplenishmentOrderItemByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindReplenishmentOrderItemByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "ReplenishmentOrderItem": %w`, err)
			}

			return entity, nil
		}
	case "Repository":
		resolverName, err := entityResolverNameForRepository(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Repository": %w`, err)
		}
		switch resolverName {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"sku\" for InventoryItem", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		return "findInventoryItemBySku", nil
	}
	return "", fmt.Errorf("%w for InventoryItem due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForPickingOrderItem(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"sku\" for PickingOrderItem", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
...
func entityResolverNameForReplenishmentOrderItem(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for ReplenishmentOrderItem", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for ReplenishmentOrderItem", ErrTypeNotFound))
			break
		}
		return "findReplenishmentOrderItemByID", nil
	}
	return "", fmt.Errorf("%w for ReplenishmentOrderItem due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForRepository(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Repository", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
