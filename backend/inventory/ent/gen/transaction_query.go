...
func (_q *TransactionQuery) QueryItem() *ItemQuery {
...
func (_q *TransactionQuery) QueryRepository() *RepositoryQuery {
...
func (_q *TransactionQuery) WithItem(opts ...func(*ItemQuery)) *TransactionQuery {
	query := (&ItemClient{config: _q.config}).Query()
...
func (_q *TransactionQuery) WithRepository(opts ...func(*RepositoryQuery)) *TransactionQuery {
	query := (&RepositoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(transaction.FieldTenantID).
//		Aggregate(gen.Count()).
//		Scan(ctx, &v)
...
//
//	client.Transaction.Query().
//		Select(transaction.FieldTenantID).
//		Scan(ctx, &v)
...
func (_q *TransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transaction, error) {
	...
	if query := _q.withRepository; query != nil {
		if err := _q.loadRepository(ctx, query, nodes, nil,
			func(n *Transaction, e *Repository) { n.Edges.Repository = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *TransactionQuery) loadItem(ctx context.Context, query *ItemQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Item)) error {
...
func (_q *TransactionQuery) loadRepository(ctx context.Context, query *RepositoryQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Repository)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Transaction)
	for i := range nodes {
...
func (_q *TransactionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withItem != nil {
			_spec.Node.AddColumnOnce(transaction.FieldItemID)
		}
		if _q.withRepository != nil {
			_spec.Node.AddColumnOnce(transaction.FieldRepositoryID)
		}
	}
...
func (_q *TransactionQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	t1.Schema(_q.schemaConfig.Transaction)
	ctx = internal.NewSchemaConfigContext(ctx, _q.schemaConfig)
