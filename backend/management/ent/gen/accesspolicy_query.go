...
type AccessPolicyQuery struct {
	...
	withRole   *RoleQuery
	withFKs    bool
	loadTotal  []func(context.Context, []*AccessPolicy) error
...
func (_q *AccessPolicyQuery) QueryRole() *RoleQuery {
...
func (_q *AccessPolicyQuery) WithRole(opts ...func(*RoleQuery)) *AccessPolicyQuery {
	query := (&RoleClient{config: _q.config}).Query()
...
func (_q *AccessPolicyQuery) prepareQuery(ctx context.Context) error {
	...
	if accesspolicy.Policy == nil {
		return errors.New("gen: uninitialized accesspolicy.Policy (forgotten import gen/runtime?)")
	}
...
func (_q *AccessPolicyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AccessPolicy, error) {
	...
	if query := _q.withRole; query != nil {
		if err := _q.loadRole(ctx, query, nodes, nil,
			func(n *AccessPolicy, e *Role) { n.Edges.Role = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *AccessPolicyQuery) loadRole(ctx context.Context, query *RoleQuery, nodes []*AccessPolicy, init func(*AccessPolicy), assign func(*AccessPolicy, *Role)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*AccessPolicy)
	for i := range nodes {
		if nodes[i].role_policies == nil {
			continue
		}
		fk := *nodes[i].role_policies
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "role_policies" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
