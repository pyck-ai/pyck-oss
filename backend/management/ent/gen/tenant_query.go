...
type TenantQuery struct {
	...
	withTenantUsers      *UserQuery
	modifiers            []func(*sql.Selector)
	loadTotal            []func(context.Context, []*Tenant) error
	withNamedTenantUsers map[string]*UserQuery
...
// QueryTenantUsers chains the current query on the "tenantUsers" edge.
func (_q *TenantQuery) QueryTenantUsers() *UserQuery {
...
// the "tenantUsers" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *TenantQuery) WithTenantUsers(opts ...func(*UserQuery)) *TenantQuery {
	query := (&UserClient{config: _q.config}).Query()
...
//
//	client.Tenant.Query().
//		GroupBy(tenant.FieldDataTypeID).
//		Aggregate(gen.Count()).
//		Scan(ctx, &v)
...
func (_q *TenantQuery) prepareQuery(ctx context.Context) error {
	...
	if tenant.Policy == nil {
		return errors.New("gen: uninitialized tenant.Policy (forgotten import gen/runtime?)")
	}
...
func (_q *TenantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tenant, error) {
	...
	if query := _q.withTenantUsers; query != nil {
		if err := _q.loadTenantUsers(ctx, query, nodes,
			func(n *Tenant) { n.Edges.TenantUsers = []*User{} },
			func(n *Tenant, e *User) { n.Edges.TenantUsers = append(n.Edges.TenantUsers, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedTenantUsers {
		if err := _q.loadTenantUsers(ctx, query, nodes,
			...
			func(n *Tenant, e *User) { n.appendNamedTenantUsers(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *TenantQuery) loadTenantUsers(ctx context.Context, query *UserQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *User)) error {
...
// WithNamedTenantUsers tells the query-builder to eager-load the nodes that are connected to the "tenantUsers"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *TenantQuery) WithNamedTenantUsers(name string, opts ...func(*UserQuery)) *TenantQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedTenantUsers == nil {
		_q.withNamedTenantUsers = make(map[string]*UserQuery)
	}
