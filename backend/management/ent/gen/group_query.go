...
func (_q *GroupQuery) QueryRoles() *RoleQuery {
...
func (_q *GroupQuery) WithRoles(opts ...func(*RoleQuery)) *GroupQuery {
	query := (&RoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupQuery) prepareQuery(ctx context.Context) error {
	...
	if group.Policy == nil {
		return errors.New("gen: uninitialized group.Policy (forgotten import gen/runtime?)")
	}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	for name, query := range _q.withNamedRoles {
		if err := _q.loadRoles(ctx, query, nodes,
			func(n *Group) { n.appendNamedRoles(name) },
			func(n *Group, e *Role) { n.appendNamedRoles(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *GroupQuery) loadRoles(ctx context.Context, query *RoleQuery, nodes []*Group, init func(*Group), assign func(*Group, *Role)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Group)
	nids := make(map[uuid.UUID]map[*Group]struct{})
	for i, node := range nodes {
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *GroupQuery) WithNamedRoles(name string, opts ...func(*RoleQuery)) *GroupQuery {
	query := (&RoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
