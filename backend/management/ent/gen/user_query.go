...
// QueryDeviceUsersUsers chains the current query on the "deviceUsersUsers" edge.
func (_q *UserQuery) QueryDeviceUsersUsers() *DeviceUserQuery {
...
// WithDeviceUsersUsers tells the query-builder to eager-load the nodes that are connected to
// the "deviceUsersUsers" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithDeviceUsersUsers(opts ...func(*DeviceUserQuery)) *UserQuery {
	query := (&DeviceUserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withDeviceUsersUsers = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	for name, query := range _q.withNamedDeviceUsersUsers {
		if err := _q.loadDeviceUsersUsers(ctx, query, nodes,
			func(n *User) { n.appendNamedDeviceUsersUsers(name) },
			func(n *User, e *DeviceUser) { n.appendNamedDeviceUsersUsers(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadDeviceUsersUsers(ctx context.Context, query *DeviceUserQuery, nodes []*User, init func(*User), assign func(*User, *DeviceUser)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(deviceuser.FieldUserID)
	}
	query.Where(predicate.DeviceUser(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.DeviceUsersUsersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
// WithNamedDeviceUsersUsers tells the query-builder to eager-load the nodes that are connected to the "deviceUsersUsers"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithNamedDeviceUsersUsers(name string, opts ...func(*DeviceUserQuery)) *UserQuery {
	query := (&DeviceUserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedDeviceUsersUsers == nil {
		_q.withNamedDeviceUsersUsers = make(map[string]*DeviceUserQuery)
	}
	_q.withNamedDeviceUsersUsers[name] = query
	return _q
}
