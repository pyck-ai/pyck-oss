...
func (_q *RoleQuery) QueryGroups() *GroupQuery {
...
func (_q *RoleQuery) QueryPolicies() *AccessPolicyQuery {
...
func (_q *RoleQuery) WithGroups(opts ...func(*GroupQuery)) *RoleQuery {
	query := (&GroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *RoleQuery) WithPolicies(opts ...func(*AccessPolicyQuery)) *RoleQuery {
	query := (&AccessPolicyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := _q.withPolicies; query != nil {
		if err := _q.loadPolicies(ctx, query, nodes,
			func(n *Role) { n.Edges.Policies = []*AccessPolicy{} },
			func(n *Role, e *AccessPolicy) { n.Edges.Policies = append(n.Edges.Policies, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range _q.withNamedGroups {
		if err := _q.loadGroups(ctx, query, nodes,
			func(n *Role) { n.appendNamedGroups(name) },
			func(n *Role, e *Group) { n.appendNamedGroups(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedPolicies {
		if err := _q.loadPolicies(ctx, query, nodes,
			func(n *Role) { n.appendNamedPolicies(name) },
			func(n *Role, e *AccessPolicy) { n.appendNamedPolicies(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *RoleQuery) loadGroups(ctx context.Context, query *GroupQuery, nodes []*Role, init func(*Role), assign func(*Role, *Group)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Role)
	nids := make(map[uuid.UUID]map[*Role]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		...
		joinT.Schema(_q.schemaConfig.GroupRoles)
...
func (_q *RoleQuery) loadPolicies(ctx context.Context, query *AccessPolicyQuery, nodes []*Role, init func(*Role), assign func(*Role, *AccessPolicy)) error {
	...
	query.Where(predicate.AccessPolicy(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.PoliciesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.role_policies
		if fk == nil {
			return fmt.Errorf(`foreign-key "role_policies" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "role_policies" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *RoleQuery) WithNamedGroups(name string, opts ...func(*GroupQuery)) *RoleQuery {
	query := (&GroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *RoleQuery) WithNamedPolicies(name string, opts ...func(*AccessPolicyQuery)) *RoleQuery {
	query := (&AccessPolicyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
