...
CREATE TABLE IF NOT EXISTS receiving.event_outbox (
    id uuid NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_event_outbox_user_created
    ON receiving.event_outbox (user_id, created_at);
...
CREATE OR REPLACE FUNCTION receiving.notify_outbox_insert()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE OR REPLACE TRIGGER trg_event_outbox_notify
    AFTER INSERT ON receiving.event_outbox
    FOR EACH ROW
    EXECUTE FUNCTION receiving.notify_outbox_insert();

-- Add comments for documentation
COMMENT ON TABLE receiving.event_outbox IS 'Transactional outbox for reliable event publishing to NATS';
COMMENT ON COLUMN receiving.event_outbox.correlation_id IS 'Links events across services, used for NATS message deduplication';
COMMENT ON COLUMN receiving.event_outbox.with_reply IS 'When true, OutboxHandler waits for workflow IDs in NATS reply';
COMMENT ON COLUMN receiving.event_outbox.retry_count IS 'Number of failed publish attempts, entry skipped after max retries';
COMMENT ON COLUMN receiving.event_outbox.workflow_ids IS 'Workflow IDs returned from workflow service, stored for audit';
COMMENT ON COLUMN receiving.event_outbox.dead_at IS 'When set, marks the correlation group as dead for manual cleanup';