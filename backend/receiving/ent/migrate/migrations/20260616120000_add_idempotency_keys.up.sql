...
CREATE TABLE IF NOT EXISTS receiving.idempotency_keys (
    id uuid NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS idempotency_keys_tenant_user_key
    ON receiving.idempotency_keys (tenant_id, user_id, key);

CREATE INDEX IF NOT EXISTS idempotency_keys_committed_created
    ON receiving.idempotency_keys (created_at)
    WHERE status = 'committed';

COMMENT ON TABLE receiving.idempotency_keys IS
