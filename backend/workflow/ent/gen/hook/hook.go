...
// The WorkflowSignalFunc type is an adapter to allow the use of ordinary
// function as WorkflowSignal mutator.
type WorkflowSignalFunc func(context.Context, *gen.WorkflowSignalMutation) (gen.Value, error)

// Mutate calls f(ctx, m).
func (f WorkflowSignalFunc) Mutate(ctx context.Context, m gen.Mutation) (gen.Value, error) {
	if mv, ok := m.(*gen.WorkflowSignalMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *gen.WorkflowSignalMutation", m)
}
