...
type WorkflowQuery struct {
	...
	withWorkflowSignals      *WorkflowSignalQuery
	loadTotal                []func(context.Context, []*Workflow) error
	modifiers                []func(*sql.Selector)
	withNamedWorkflowSignals map[string]*WorkflowSignalQuery
...
// QueryWorkflowSignals chains the current query on the "workflowSignals" edge.
func (_q *WorkflowQuery) QueryWorkflowSignals() *WorkflowSignalQuery {
...
func (_q *WorkflowQuery) Clone() *WorkflowQuery {
	...
	return &WorkflowQuery{
		...
		withWorkflowSignals: _q.withWorkflowSignals.Clone(),
...
// WithWorkflowSignals tells the query-builder to eager-load the nodes that are connected to
// the "workflowSignals" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *WorkflowQuery) WithWorkflowSignals(opts ...func(*WorkflowSignalQuery)) *WorkflowQuery {
	query := (&WorkflowSignalClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withWorkflowSignals = query
	return _q
}
...
//		GroupBy(workflow.FieldTenantID).
//		Aggregate(gen.Count()).
//		Scan(ctx, &v)
...
func (_q *WorkflowQuery) prepareQuery(ctx context.Context) error {
	...
	if workflow.Policy == nil {
		return errors.New("gen: uninitialized workflow.Policy (forgotten import gen/runtime?)")
	}
	if err := workflow.Policy.EvalQuery(ctx, _q); err != nil {
		return err
	}
	return nil
}
...
func (_q *WorkflowQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Workflow, error) {
	...
	_spec.Node.Schema = _q.schemaConfig.Workflow
	...
	if query := _q.withWorkflowSignals; query != nil {
		if err := _q.loadWorkflowSignals(ctx, query, nodes,
			func(n *Workflow) { n.Edges.WorkflowSignals = []*WorkflowSignal{} },
			func(n *Workflow, e *WorkflowSignal) { n.Edges.WorkflowSignals = append(n.Edges.WorkflowSignals, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedWorkflowSignals {
		if err := _q.loadWorkflowSignals(ctx, query, nodes,
			func(n *Workflow) { n.appendNamedWorkflowSignals(name) },
			func(n *Workflow, e *WorkflowSignal) { n.appendNamedWorkflowSignals(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *WorkflowQuery) loadWorkflowSignals(ctx context.Context, query *WorkflowSignalQuery, nodes []*Workflow, init func(*Workflow), assign func(*Workflow, *WorkflowSignal)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(workflowsignal.FieldWorkflowID)
	}
	query.Where(predicate.WorkflowSignal(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(workflow.WorkflowSignalsColumn), fks...))
	}))
...
func (_q *WorkflowQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	t1.Schema(_q.schemaConfig.Workflow)
	ctx = internal.NewSchemaConfigContext(ctx, _q.schemaConfig)
...
// WithNamedWorkflowSignals tells the query-builder to eager-load the nodes that are connected to the "workflowSignals"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *WorkflowQuery) WithNamedWorkflowSignals(name string, opts ...func(*WorkflowSignalQuery)) *WorkflowQuery {
	query := (&WorkflowSignalClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedWorkflowSignals == nil {
		_q.withNamedWorkflowSignals = make(map[string]*WorkflowSignalQuery)
	}
	_q.withNamedWorkflowSignals[name] = query
	return _q
}
