...
CREATE INDEX IF NOT EXISTS idx_event_outbox_correlation
    ON workflow.event_outbox (correlation_id, created_at)
    WHERE published_at IS NULL AND dead_at IS NULL;

-- Index for user-specific event queries
CREATE INDEX IF NOT EXISTS idx_event_outbox_user_created
    ON workflow.event_outbox (user_id, created_at);
...
CREATE OR REPLACE FUNCTION workflow.notify_outbox_insert()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE OR REPLACE TRIGGER trg_event_outbox_notify
    AFTER INSERT ON workflow.event_outbox
    FOR EACH ROW
    EXECUTE FUNCTION workflow.notify_outbox_insert();

-- Add comments for documentation
COMMENT ON TABLE workflow.event_outbox IS 'Transactional outbox for reliable event publishing to NATS';
COMMENT ON COLUMN workflow.event_outbox.correlation_id IS 'Links events across services, used for NATS message deduplication';
COMMENT ON COLUMN workflow.event_outbox.with_reply IS 'When true, OutboxHandler waits for workflow IDs in NATS reply';
COMMENT ON COLUMN workflow.event_outbox.retry_count IS 'Number of failed publish attempts, entry skipped after max retries';
COMMENT ON COLUMN workflow.event_outbox.workflow_ids IS 'Workflow IDs returned from workflow service, stored for audit';
COMMENT ON COLUMN workflow.event_outbox.dead_at IS 'When set, marks the correlation group as dead for manual cleanup';
