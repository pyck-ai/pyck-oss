...
func main() {
	...
	if err := workflowRouter.Start(ctx); err != nil {
		log.ForContext(ctx).Fatal().
			Err(err).
			Msg("failed to start workflow router")
		return
	}
	...
	httpRouter.Handle("/health", handlers.NewHealthCheckHandler(db.NewDbHealthChecker(pgxDriver.DB(), entworkflow.Table)))
